...
# Lorenzo Giannuzzo: Incremento perdite per effetto Joule e polarizzazione elettrodo
LITHIUM_ION_1C_DATA = {
...
- Dati commerciali Tesla Megapack (LFP)
...
# ========================================================================================================
# SEZIONE 3: CLASSI DI SUPPORTO PER EFFICIENZA
# ========================================================================================================
...
class PSOOptimizer:
    ...
    def _evaluate(self, battery, actions, prices):
        ...
        CORRETTO v2.2.0: Il costo del degrado viene applicato sia in carica che in scarica,
        poiché ogni operazione consuma un ciclo di vita della batteria.

        Formula degrado per OGNI operazione (carica O scarica):
            degradation_cost = abs(energy) * DEGRADATION_COST_PER_MWH / (2 * EOL_cycles)

        Il divisore "2" è corretto perché:
        - 1 ciclo completo = 1 carica + 1 scarica
        - Ogni singola operazione consuma 0.5 cicli
        - Quindi: costo_singola_operazione = costo_totale / (2 * cicli_vita)
        ...
        for power, price in zip(actions, prices):
            if power > 0:  # CARICA
                ...
                if actual_power > 0.01:
                    ...
                    # CORREZIONE v2.2.0: Penalizzazione degrado in CARICA
                    # Diviso per 2 perché la carica è solo metà di un ciclo completo
                ...
                if actual_power > 0.01:
                    ...
                    # CORREZIONE v2.2.0: Penalizzazione degrado in SCARICA
                    # Diviso per 2 perché la scarica è solo metà di un ciclo completo
...
def create_visualizations(results_df, battery):
    ...
    # Crea figura con 12 subplot (4 righe x 3 colonne)
