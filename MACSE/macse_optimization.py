...
# Parametri Degrado Batteria
DEGRADATION_COST_PER_MWH = 25.0  # €/MWh di throughput (per ottimizzatore)

# Parametri MACSE
MACSE_ENABLED = True  # Abilita partecipazione al MACSE
MACSE_CAPACITY_PERCENTAGE = 0.3  # % capacità allocata al MACSE (30%)
MACSE_CONTRACT_YEARS = 1  # Durata contratto MACSE (anni)
MACSE_PRICE_PER_MW_YEAR = 50000  # €/MW/anno (da asta MACSE)
MACSE_AVAILABILITY_TARGET = 0.95  # Target disponibilità (95%)
MACSE_PENALTY_THRESHOLD = 0.92  # Soglia penale disponibilità (92%)
MACSE_BONUS_THRESHOLD = 0.98  # Soglia bonus disponibilità (98%)
MACSE_PENALTY_MULTIPLIER = 1.5  # Moltiplicatore penale
MACSE_BONUS_PERCENTAGE = 0.10  # Bonus 10% se Avail >= 98%
MACSE_RESPONSE_TIME_MINUTES = 15  # Tempo di risposta richiesto (minuti)
MACSE_MIN_SOC_FOR_DISCHARGE = 0.20  # SOC minimo per scarica MACSE
MACSE_MAX_SOC_FOR_CHARGE = 0.80  # SOC massimo per carica MACSE
# ==================================================================
...
class PSOOptimizer:
    """PSO ottimizzato con inizializzazione intelligente"""
...
class RollingHorizonSimulator:
    ...
    def simulate(self, prices_df):
        ...
        if MACSE_ENABLED:
            print(f"\n🔵 MACSE ABILITATO:")
            print(
                f"  - Capacità allocata MACSE: {self.battery.macse_capacity:.2f} MWh ({MACSE_CAPACITY_PERCENTAGE * 100}%)")
            print(
                f"  - Capacità per trading: {self.battery.trading_capacity:.2f} MWh ({(1 - MACSE_CAPACITY_PERCENTAGE) * 100}%)")
...
def generate_technical_report(results_df, battery, gross_profit, degradation_cost,
    ...
    with PdfPages(pdf_path) as pdf:
        ...
        # Box centrato con informazioni chiave
        box_lines = [
        ...
        # Disegna rettangolo attorno al box
        ...
        y_pos = 0.86  # Aumentato margine superiore
        ...
        box_y_trading = y_pos + 0.075  # Posizione corretta del box
        ...
        # CORREZIONE: Calcolo profitto mensile OPERATIVO (senza costo degrado)
        # Calcola il profitto operativo orario (vendite - acquisti, SENZA degrado)
