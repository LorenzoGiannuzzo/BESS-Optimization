...
Last Update of current code: 20/02/2025 """

# IMPORT LIBRARIES
import pandas as pd
from argparser_s import input_sellprice_path, input_buyprice_path
...
if weekends == 'True':
    ...
    PUN_timeseries_sell = final_result_weekdays.to_numpy()

else:
    PUN_timeseries_sell = final_result_weekends.to_numpy()
...
if weekends == 'True':
    # Convert to numpy arrays if needed
    PUN_timeseries_buy = final_result_weekdays.to_numpy()

else:
    PUN_timeseries_buy = final_result_weekends.to_numpy()

# TODO BRUTE FORCE PUN TO BE THE ONE EVALUATED FOR WEEKDAYS, SHOULD BE FIXED
PUN_timeseries_buy = final_result_weekdays.to_numpy()
time_window = len(PUN_timeseries_buy[:, 1])
